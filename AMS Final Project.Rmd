...
|   **Life Satisfaction by Country with Average Scores** : This box plot shows the distribution of life satisfaction scores across different countries, with their respective average scores annotated in red.
```{r, echo=FALSE}
...
|   **Correlation Heatmap of Metrics** :This heatmap reveals the correlation between various metrics in the dataset.
...
|   **Radar Chart for All 15 Countries** : This radar chart compares the average scores of 15 countries across multiple attributes such as Education, Jobs, Income, Housing, and Environment.
...
max_vals <- rep(10, ncol(radar_data) - 1)  # Set max values (10 for all attributes)
min_vals <- rep(0, ncol(radar_data) - 1)   # Set min values (0 for all attributes)
...
radarchart(radar_chart_data, axistype = 1,
           pcol = country_colors[radar_data$Country],  # Colors for outlines
...
par(mar = c(5, 4, 4, 8), xpd = TRUE)  # Add extra space on the right side
legend("topright", inset = c(-0.2, 0),   # Adjust position outside the plot
       legend = radar_data$Country,      # Country names
       col = country_colors[radar_data$Country],
       lty = 1, lwd = 3, cex = 0.8,      # Adjust font size
       ncol = 1,                         # Single column
       title = "Countries")

```

|   **Interactive Plot: Different Shapes by Country, Color by Life Satisfaction** : This interactive scatter plot shows the relationship between Income and Civic Engagement, with shapes representing countries and colors indicating life satisfaction levels.
```{r, echo=FALSE, warning=FALSE, message=FALSE}
...
plot1_interactive <- ggplotly(plot1, tooltip = "text")
